...
### for plotting ggplot geoms in alignment with trees


# This little function retrieves the y psotions of the tree tips
tree_y <- function(ggtree, data, data_label = "label") {
  if (!inherits(ggtree, "ggtree"))
    stop("not a ggtree object")

  left_join(select(data, !!data_label), select(ggtree$data, label, y), by = setNames("label", data_label)) %>%
    pull(y)
}
...
ggtreeplot <- function(ggtree, data = NULL, data_label = "label", mapping = aes(), flip=FALSE, expand_limits = expand_scale(0,.6), ...){
  ...
  if(flip){
    mapping <- modifyList(aes(x=x), mapping)
    data <- mutate(data, x=tree_y(ggtree, data, !!data_label))
    gg <- ggplot(data=data, mapping = mapping, ...) +
      scale_x_continuous(limits=limits, expand=c(0,0))
  }else{
    mapping <- modifyList(aes(y=y), mapping)
    data <- mutate(data, y=tree_y(ggtree, data, !!data_label))
    gg <- ggplot(data=data, mapping = mapping, ...) +

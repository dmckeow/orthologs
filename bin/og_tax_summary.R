...
# Conert the % species filter to the number of species required
...
for(i in 2:ncol(ogs)){
    ...
    # And the taxonomic group for species (id - was orignially col species)
    grp <- unique(as.character(samples$taxonomy[which(samples$id == spp)]))

...
output: 
  html_document:
    output_file: "../results/summary.html"
    toc: true
    toc_float: TRUE
    toc_depth: 4
...
SAMPLESHEET <- "/users/asebe/dmckeown/projects/crg-bcaortho/ioo_mz46-samplesheet1.csv"
...
## Pipeline workflow
Current overview of the pipeline
...
mermaid("
graph TB
  PREFILTER-->PREFILTER_SEARCH
  PREFILTER-->ORTHOFINDER_PREP_ALL
  PREFILTER_SEARCH-->CONCATENATE_FASTAS
  CONCATENATE_FASTAS-->ORTHOFINDER_PREP_ALL
  ORTHOFINDER_PREP_ALL-->DIAMOND_BLASTP_ALL
  DIAMOND_BLASTP_ALL-->ORTHOFINDER_MCL_ALL
  ORTHOFINDER_MCL_ALL-->FILTER_ORTHOGROUPS
  FILTER_ORTHOGROUPS-->ALIGN_SEQS
  FILTER_ORTHOGROUPS-->ALIGN_REMAINING_SEQS
  ALIGN_SEQS-->INFER_TREES
  ALIGN_REMAINING_SEQS-->INFER_REMAINING_TREES
  INFER_TREES-->ASTEROID
  ASTEROID-->SPECIESRAX
  SPECIESRAX-->GENERAX_PER_FAMILY
  INFER_REMAINING_TREES-->GENERAX_PER_SPECIES
  SPECIESRAX-->GENERAX_PER_SPECIES
  INFER_REMAINING_TREES-->GENERAX_PER_FAMILY
  GENERAX_PER_FAMILY-->ORTHOFINDER_PHYLOHOGS
  GENERAX_PER_SPECIES-->ORTHOFINDER_PHYLOHOGS
...
# Initialize an empty list to store count_dat for each group (for testing)
count_dat_list <- list()
num_spp_list <- list()
...
for(i in 1:length(grps[order(grps)])){
  ...
  # Modified count_dat generation
  count_dat <- og_counts[which(og_counts$Orthogroup %in% rates_per_og$gene_family), c("Orthogroup", spps), drop = FALSE]
  count_dat <- count_dat[, -1, drop = FALSE]  # Remove the Orthogroup column
  count_dat_list[[grp]] <- count_dat

process WITCH {
    ...
    stageInMode = 'copy'
    publishDir(
        path: "${params.outdir}/${publish_subdir}/witch_alignments",
...
process WITCH_ARRAY {
    tag { "Array: ${fasta_list.collect().size()} files" }
    label 'process_witch'
    ...
    def min_len = params.min_ungapped_length ?: '20'
    """
    # Loop through the fasta files in the input list
    for fasta in ${fasta_list}; do
        ...
        # Reorganize results for publishing
        mkdir -p original_alignments
        mkdir -p cleaned_alignments
        mv alignments/merged.fasta original_alignments/\$(basename \$fasta .fa)_witch.fa
        mv final_masked.fasta cleaned_alignments/\$(basename \$fasta .fa)_witch_cleaned.fa
        rm -r alignments/ && rm tmp.fasta

        # In the rare case that this filtering reduces sequences down to < 4
        # sequences, delete the output cleaned alignments to exclude them from
        # downstream phylogenetic analyses.
        n_remain=\$(grep ">" cleaned_alignments/\$(basename \$fasta .fa)_witch_cleaned.fa | wc -l)
        if [ \$n_remain -lt 4 ]; then
            rm cleaned_alignments/\$(basename \$fasta .fa)_witch_cleaned.fa
        else
            # Now pull out the sequences, and split into a TreeRecs format mapping
            # file, where each protein in the tree is a new line, listing species
            # and then the protein
            mkdir -p species_protein_maps
            grep ">" cleaned_alignments/\$(basename \$fasta .fa)_witch_cleaned.fa | sed "s/>//g"  | sed "s/.*://g" > prot
            sed -E 's/___.+//g' prot > spp
            paste prot spp > species_protein_maps/\$(basename \$fasta .fa)_map.link
            rm prot && rm spp
        fi
    done
